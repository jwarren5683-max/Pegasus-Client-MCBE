...
1. Fully close Minecraft so no older Loki DLL remains loaded.
2. Extract the entire GitHub artifact into a new folder.
3. Start Minecraft Bedrock and leave it at the title screen.
4. Run `Loki.exe` with the same permission level as Minecraft and load the mod.
...
The optional `tools\BedrockCompatibilityScanner.exe` performs the same kind of read-only scan from outside the game. If Windows reports access denied, run it with the same permission level used for Loki.
