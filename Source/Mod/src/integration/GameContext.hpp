...
inline void observe_game_mode(void* game_mode) noexcept {
    if (!game_context_detail::readable(game_mode, 16)) return;
    void* candidate = *reinterpret_cast<void**>(static_cast<std::byte*>(game_mode) + sizeof(void*));
    if (!game_context_detail::readable(candidate, sizeof(void*))) return;

    const auto build = current_bedrock_build();
    // The known vtable identities below are valid only for the legacy 1.26.45
    // executable. Unknown builds intentionally stay fail-closed until their
    // identities/layouts have been independently validated.
    if (!is_legacy_12645_build(build)) return;
