...
internal static class Program {
    ...
    [STAThread] static int Main(string[] args) {
        ...
        Application.Run(new LokiForm()); return 0;
    }
...
internal sealed class LokiButton : Button {
    protected override void OnPaint(PaintEventArgs e) {
        e.Graphics.Clear(Enabled ? BackColor : Color.FromArgb(18, 30, 48));
        TextRenderer.DrawText(e.Graphics, Text, Font, ClientRectangle,
            Enabled ? Color.White : Color.FromArgb(125, 151, 181), TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
        if (Focused) ControlPaint.DrawFocusRectangle(e.Graphics, Rectangle.Inflate(ClientRectangle, -4, -4));
    }
}

internal sealed class LokiForm : Form {
    readonly Color muted = Color.FromArgb(139, 165, 194);
    readonly ComboBox targets = new ComboBox();
    readonly Label status = new Label();
    readonly Button load = new LokiButton();
    readonly Button refresh = new LokiButton();
    sealed class Target {
    ...
    internal LokiForm() {
        ...
        AddLabel("UTILITY MOD   /   1.0", 36, 93, 440, 25, 10, Color.FromArgb(79, 166, 255), FontStyle.Bold);
        ...
        targets.BackColor = Color.FromArgb(12, 22, 35); targets.ForeColor = Color.White; targets.FlatStyle = FlatStyle.Flat;
        ...
        targets.DrawItem += delegate(object sender, DrawItemEventArgs e) {
            using (var brush = new SolidBrush(Color.FromArgb(12,22,35))) e.Graphics.FillRectangle(brush, e.Bounds);
        ...
        SetupButton(refresh, "Refresh", 438, 251, 106, 34, Color.FromArgb(20, 38, 60));
        refresh.Click += delegate { RefreshTargets(); };
        SetupButton(load, "Load Loki   →", 36, 309, 508, 49, Color.FromArgb(0, 105, 220));
        ...
        Controls.Add(status);
        AddLabel("TESTED LOKI BUILD", 36, 451, 300, 19, 8, Color.FromArgb(79, 166, 255), FontStyle.Bold);
    ...
    protected override void OnPaint(PaintEventArgs e) {
        base.OnPaint(e);
        using (var brush = new LinearGradientBrush(new Rectangle(0,0,Width,5), Color.FromArgb(0,82,204), Color.FromArgb(44,181,255), 0f)) e.Graphics.FillRectangle(brush,0,0,Width,5);
        using (var pen = new Pen(Color.FromArgb(25,48,72))) { e.Graphics.DrawLine(pen,36,207,544,207); e.Graphics.DrawLine(pen,36,440,544,440); }
    }
    void RefreshTargets() {
        targets.Items.Clear();
