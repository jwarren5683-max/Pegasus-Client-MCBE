...
1. Keep `Loki.exe` and `BedrockUtilityFramework.Xray.dll` together in this folder.
...
3. Open `Loki.exe`. Select your game session, then choose **Load Loki**. Use Refresh if you started Minecraft afterward.
...
- If access is denied, run Loki with the same privilege level as Minecraft or administrator privileges.

## Release contents and source provenance

- `BedrockUtilityFramework.Xray.dll`: current tested Enhanced mod build.
- `Loki.exe`: x64 desktop injector with a blue-and-black GUI. It validates the bundled DLL's SHA-256 and uses the standard Windows DLL loader. It also supports `--inject` and a disposable-process `--self-test`.
...
The script uses the Windows .NET Framework compiler and writes `Loki.exe` here. No external packages are required. If intentionally distributing a different DLL, update `ExpectedHash` in the injector source before rebuilding.
...
cmake --build C:/loki-build --config Release
ctest --test-dir C:/loki-build -C Release --output-on-failure
```

The output is `C:/loki-build/Release/BedrockUtilityFramework.Xray.dll`. Update the injector's pinned hash and rebuild the injector whenever replacing this DLL.
